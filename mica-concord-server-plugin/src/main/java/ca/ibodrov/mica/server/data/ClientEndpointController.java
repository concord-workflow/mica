...
public class ClientEndpointController {
    ...
    @Inject
    public ClientEndpointController(@MicaDB DSLContext dsl,
    ...
    public void importFromClientData(UUID profileId) {
        ...
        var profile = dsl.selectFrom(MICA_PROFILES).where(MICA_PROFILES.ID.eq(profileId))
                .fetchOptional()
                .orElseThrow(() -> ApiException.badRequest("Unknown profile ID: " + profileId));

        var latestData = dsl.select(MICA_CLIENT_DATA.EXTERNAL_ID, MICA_CLIENT_DATA.PARSED_DATA)
        ...
        for (Record2<String, JSONB> data : latestData) {
            var clientId = dsl.select(MICA_CLIENTS.ID).from(MICA_CLIENTS).where(MICA_CLIENTS.NAME.eq(data.value1()))
            ...
            endpointUri.ifPresent(uri -> dsl.transaction(tx -> {
